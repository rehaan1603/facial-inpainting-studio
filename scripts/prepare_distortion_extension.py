...
def main():
    ...
    out=ROOT/'outputs/distortion_extension_v1';out.mkdir(exist_ok=False)
    ...
    for identity in sorted([i for i,g in groups.items() if len(g)>=8],key=lambda i:rank(i,'distortion_extension_v1')):
        assert identity not in RESERVED and identity not in excluded
        kept=[];attempt={'identity':identity,'photos':[]}
        for row in sorted(groups[identity],key=lambda r:rank(r['hq_id'],'distortion_extension_photo')):
    ...
    cfg={'version':1,'date':'2026-09-20','stage':'Fresh development confirmation, never final test','identities':[g['identity'] for g in selected],
         'kinds':KINDS,'strengths':[.5,.99],'scales':[.8],'seeds':[29],'steps':30,'guidance_scale':5.,'blend':'poisson','resolution':512,
         'severities':['medium','severe'],'mask_schedule':{'medium':'central_face','severe':'half_face'},
         'reference_variation':'First reference JPEG quality 20, remaining three unchanged; all references remain same-person. Not an isolated reference-quality ablation.',
         'preservation_confidence':.5,'statistical_unit':'identity','primary_contrasts':['low minus high strength on five partially degraded types','preserved high minus standard high on five partially degraded types'],
         'primary_metrics':['facenet_cosine','hole_mae'],'multiplicity':'Holm over four identity-averaged primary tests; medium/severe strata and observed-input comparisons retained',
         'selection':'Fixed hash ranking; val split only; exclude all earlier training, inspected, attempted and reserved identity labels; exact and pHash<=6 duplicate exclusion; one-face detector screening',
         'source_sha256':sha(source),'fingerprints_sha256':sha(fingerprints),'pretraining_overlap':'unknown','final_test_used':False,
         'interpretation':'Mask geometry and severity vary together; do not attribute cross-stratum differences solely to severity. Within-case method comparisons are paired. No promotion if output loses target fidelity against unchanged input.'}
    write_new(ROOT/'research/protocols/distortion_extension_v1.json',cfg)
    ...
    for group in selected:
        ...
        for i,row in enumerate(group['images']):
            ...
            path=folder/f'role_{i}.png';image.save(path)
            if i==1:
                compressed=folder/'reference_1_jpeg20.jpg';image.save(compressed,quality=20)
                with Image.open(compressed) as im:im.convert('RGB').save(folder/'reference_1_degraded.png')
                path=folder/'reference_1_degraded.png'
            roles.append({'path':str(path),'sha256':sha(path)})
        ...
        for severity in cfg['severities']:
            region=cfg['mask_schedule'][severity];mask=damage_mask(clean.shape[:2],region,severity,29,geometry['bbox'],geometry['landmarks'])
            for kind in KINDS:
                observed,_,metadata=degrade(clean,kind,severity,29,mask=mask,region=region)
                cid=f'{identity}_{kind}_{severity}';dest=out/'cases'/cid;dest.mkdir(parents=True)
                Image.fromarray(observed).save(dest/'observed.png');Image.fromarray(mask.astype('uint8')*255).save(dest/'mask.png');np.save(dest/'confidence.npy',evidence_map(mask,kind=='removal',.5))
                c={'case_id':cid,'identity':identity,'split':'development_confirmation','kind':kind,'severity':severity,'references':roles[1:5],'distortion':metadata}
                for name in ['observed','mask','confidence']:
                    c[name]=str(dest/(name+('.npy' if name=='confidence' else '.png')));c[name+'_sha256']=sha(c[name])
                inference.append(c);evaluation.append({'case_id':cid,'identity':identity,'target':roles[0],'gallery':roles[5:]})
    write_new(out/'inference_manifest.json',{'protocol_sha256':sha(ROOT/'research/protocols/distortion_extension_v1.json'),'cases':inference})
    write_new(out/'evaluation_manifest.json',{'cases':evaluation,'clean_data_scope':'Scoring only'})
    print('48 cases, 96 requested generations; four fresh development identities; no final images opened',flush=True)
