...
from distortion_aware_study import RESERVED
BASE=ROOT/'outputs/unfamiliar_studio_v2'
OUT=ROOT/'outputs/unfamiliar_studio_audit_v2'
PROTOCOL=ROOT/'research/protocols/unfamiliar_studio_audit_v2.json'
MODES=[('lama','lama','standard',None),('resshift','resshift','standard',None),
       ('reference_256','reference','compact',None),('reference_512','reference','standard',None),('reference_1024','reference','detailed',None),
       ('selection_256','reference_select','compact',None),('selection_512','reference_select','standard',None),('selection_1024','reference_select','detailed',None),
       ('missing_evidence_low','reference','standard',.5),('missing_evidence_high','reference','standard',.99)]
...
def cases():
    ...
    assert len(selected)==4 and {c['identity'] for c in selected}==set(json.loads((ROOT/'research/protocols/unfamiliar_studio_v2.json').read_text())['identities'])
...
def generate():
    ...
    for c in cases():
        ...
        for arm,backbone,detail,strength in MODES if c['kind']=='removal' else PARTIAL:
            ...
            try:
                payload=dict(image=png(c['observed'],c['observed_sha256']),mask=png(c['mask'],c['mask_sha256']),backbone=backbone,detail=detail,mode='painted',blend='poisson',neutralize=False)
                ...
                if strength is not None:
                    ...
                    weights=np.where(mask,0 if c['kind']=='removal' else 128,255).astype('uint8');stream=io.BytesIO();Image.fromarray(weights).save(stream,format='PNG')
    ...
    write_new(OUT/'generation_complete.json',{'protocol_sha256':sha(PROTOCOL),'expected_rows':30})
