...
from unfamiliar_studio_audit_v2 import OUT,BASE,PROTOCOL,cases
...
def main():
    ...
    result=dict(scope='Two newly selected local development identities (3182, 1037). Pretrained exposure unknown. No population accuracy or novelty claim.',protocol_sha256=sha(PROTOCOL),evaluation_sha256=sha(OUT/'evaluation.json'),expected_rows=30,scored_rows=sum(r['evaluation']['status']=='complete' for r in rows),groups=groups,paired_observed=pairs,rows=rows,final_test_used=False)
    ...
    write_new(ROOT/'research/unfamiliar_studio_accuracy_v2.json',result)
    lines=['# Studio generation accuracy audit — 24 September 2026','',result['scope'],'',
           f"Completed scores: **{result['scored_rows']}/30**, including four unchanged-image controls. Planned generations: **26**. No quality-based retries or discarded failures.",'',
    ...
    with (ROOT/'research/UNFAMILIAR_STUDIO_ACCURACY_V2.md').open('x',encoding='utf-8') as stream:
        stream.write('\n'.join(lines)+'\n')
