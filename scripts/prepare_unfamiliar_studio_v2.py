...
def main():
    ...
    out=ROOT/'outputs/unfamiliar_studio_v2';out.mkdir(exist_ok=False)
    ...
    earlier=json.loads((ROOT/'outputs/unfamiliar_smoke_v1/selection.json').read_text())
    excluded |= {str(a['identity']) for a in earlier['attempts']} | {str(a['identity']) for a in earlier['selected']}
    ...
    for identity in sorted([i for i,g in groups.items() if len(g)>=8],key=lambda i:rank(i,'unfamiliar_studio_v2')):
        assert identity not in RESERVED and identity not in excluded
        kept=[];attempt={'identity':identity,'photos':[]}
        for row in sorted(groups[identity],key=lambda r:rank(r['hq_id'],'unfamiliar_studio_photo_v2')):
    ...
    cfg={'date':'2026-09-24','identities':[g['identity'] for g in selected],'purpose':'unfamiliar local development studio accuracy audit, not final or pretraining-disjoint validation','selection':'fixed hash order, exclude all earlier protocol, extension and unfamiliar-smoke attempted/selected identities, exact and pHash<=6 duplicate filtering, one detected face, eight photos per identity','conditions':['removal','mixed'],'seed':17,'steps':50,'guidance':1.5,'final_test_used':False}
    write_new(ROOT/'research/protocols/unfamiliar_studio_v2.json',cfg)
