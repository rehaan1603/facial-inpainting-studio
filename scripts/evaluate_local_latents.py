"""Score native local-feature generation using the unchanged independent metrics."""
...
def main():
    ...
    while not (OUT/'comparison.json').exists():
        if time.monotonic()>deadline:raise TimeoutError('Local generation incomplete')
        time.sleep(5)
    ...
    for row in data['rows']:
        ...
        if dest.exists():score=json.loads(dest.read_text())
            ...
            try:
                ...
                values=metrics.score(output,target,observed,mask,tv);ov,od=metrics.features(output)
        ...
        rows.append(dict(row,evaluation=score));print('LOCAL SCORE',len(rows),'/48',score['status'],flush=True)
